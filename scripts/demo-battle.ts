...
function show(c: Character, label: string) {
  ...
  if (c.statuses.length) console.log(`  状态: ${c.statuses.map(s => `${s.type}×${s.stacks}`).join(', ')}`)
}

// —— 锻体拳法 build：开局全属性+1 ——
const p = new Character('p1', '玩家·拳', { strength: 14, vitality: 12, dexterity: 10, technique: 10, insight: 8, wisdom: 6 })
p.forgingLevel = 4
for (const attr of (['strength','vitality','dexterity','technique','insight','wisdom'] as const)) p.attrs.modify(attr, 1)
p.actions = ['iron_charge', 'straight_punch', 'crushing_blow', 'power_double']
p.skills = ['铁布衫']
