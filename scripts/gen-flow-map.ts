...
const HEADERS: { name: string; storyId: string }[] = [
    { name: '少年行', storyId: 'sect' },
    { name: '夜行者', storyId: 'veteran' },
    { name: '朝花夕拾', storyId: 'wanderer' },
    { name: '断刀', storyId: 'feud' },
    { name: '双生祭', storyId: 'xuanmen' },
]

// ── 池事件分类 ──
/** 四支线链（按解锁顺序展示：偶遇→结拜→青山论剑→酒吧杀人→归海楼） */
const CHAIN_ORDER = [
    'chronicle_tavern_encounter', // 路见不平
    'chronicle_sworn_brothers', // 九朵桃花结义（结拜）
    'chronicle_six_duel', // 青山之巅论剑（排在酒吧杀人之前：重逢笔墨在先）
    'chronicle_bar_killing', // 九朵桃花酒吧杀人事件
    'chronicle_guihailou', // 归海楼研讨会
]
...
const REMARKS: Record<number, string> = {
    ...
    4: '成长池（夜行者 = 正式训练）',
    5: '成长池（夜行者 = 正规训练）',
    6: '成长池（夜行者 = 入伍；双生祭 = 结识小树）',
    7: '成长池（少年行 = 走火入魔·师兄相救；夜行者 = 同袍）',
    8: '成长池（夜行者 = 夜行者分岔路）',
    9: '成长池（少年行 = 约定下山；双生祭 = 家传密辛）',
    11: '一阶段Boss（少年行 = 师兄弟对决 / 双生祭 = 玄九生死斗，其余随机对手）',
    12: '支线链/回忆池（断刀 = 加入调查科；双生祭 = 取名玄久）',
    13: '支线链/回忆池（断刀 = 阿九到来；双生祭 = 家族内斗）',
    14: '支线链/回忆池（朝花夕拾 = 青山之巅论剑·与陶朵重逢；少年行/双生祭 = 归海楼研讨会；断刀 = 现场勘查）',
    15: '支线链/回忆池（少年行 = 归海楼·表演赛；双生祭 = 小树；朝花夕拾 = 与陶朵相处；断刀 = 与阿九相处）',
    16: '支线链/回忆池（朝花夕拾 = 九朵桃花酒吧杀人事件·后门；少年行 = 重逢 / 双生祭 = 质问；夜行者 = 兄弟之死）',
    17: '支线链/回忆池（夜行者 = 追查；双生祭 = 河边修炼）',
    18: '支线链/回忆池（朝花夕拾 = 恩师问话；断刀 = 发现真相）',
    19: '支线链/回忆池（少年行 = 追踪；夜行者 = 李雪影）',
    20: '支线链/回忆池（朝花夕拾 = 来风报信）',
    21: '支线链/回忆池（断刀 = 内心挣扎；双生祭 = 决定参赛）',
    22: '守门人（断刀 = 阿九）',
...
/** 朝花夕拾流主线链的 flag 进度：按节点推算（结拜 n12 → 青山论剑 n14 → 酒吧 n16 → 恩师 n18 → 来风 n20） */
 ...
 *  各线渲染池事件只在本故事列展示；
 *  共享链的青山论剑/酒吧杀人已为朝花夕拾让位、归海楼研讨会已为少年行/双生祭让位（各自走主线版）；
...
lines.push('> 池事件均为 3 选 1 候选：可空缺、无兜底。四支线链顺序：路见不平 → 九朵桃花结义（结拜）→ 青山之巅论剑 → 九朵桃花酒吧杀人事件 → 归海楼研讨会（归海楼研讨会对少年行/双生祭是主线，不在池中）。', '')
