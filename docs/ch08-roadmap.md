...
**目标：** 触发器装备/触发、义体植入/惩罚、锻体炁技、御物本命物

**依赖：** Phase 1

| 任务                       | 测试点                         | 预估      |
| -------------------------- | ------------------------------ | --------- |
| 触发器系统                 | 条件匹配时触发，消耗 AP        | 1d        |
| 触发器槽位                 | `wisdom` 决定槽数              | 0.5d      |
| 义体系统                   | 属性加成 + 部位惩罚 + 叠加规则 | 1d        |
| 锻体·炁技                  | 8 个炁技的数据 + 触发逻辑      | 1.5d      |
| 御物·本命物                | 5 件本命物 + 等级自动成长      | 1d        |
| 灼烧/麻痹/失衡/中毒 等状态 | 叠层/衰减/驱散                 | 1d        |
| CC 递减（DR）              | 2 分钟窗口内的递减             | 0.5d      |
| **合计**                   |                                | **~6.5d** |
