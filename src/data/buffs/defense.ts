...
export const DEFENSE_BUFFS: BuffDef[] = [
    ...
    },
    {
        ...
        name: '混元炁',
        description:
            '混元护体：受到超过8点或炁伤害时护体 —— 1m 内反伤所受伤害的三分之一并击退对手，缠耗与反伤等量，自身仍承受全额伤害；1m 外以缠抵伤，消耗伤害三分之一的缠、减掉三分之一伤害，缠不够就按能付的减，1 缠抵 1 伤。',
        tags: ['defense'],
        expiry: { type: 'permanent' },
        stacking: { type: 'none' },
        onTakeDamage: ({ final, attacker, target, engine, state, source }) => {
            ...
            if (dist > 1) {
                ...
                engine.emitLog({
                    type: 'system',
                    message: `[混元炁] ${target.name}耗${pay}缠抵掉${pay}点远程伤害（${round1(final)}→${reduced}）`,
                    actorId: target.id,
                })
                return reduced
            }
