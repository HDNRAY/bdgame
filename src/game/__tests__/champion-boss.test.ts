...
/** 一个 build 挂上的奇物带来的属性加成合计（只看 stat_buff 的 attrs，够本测试用） */
function artifactGrants(build: CharacterBuild): Partial<Record<AttrName, number>> {
    ...
    for (const r of build.rewards) {
        ...
        for (const e of def?.effects ?? []) {
            const attrs = (e as { attrs?: Record<string, number> }).attrs
            if (!attrs) continue
