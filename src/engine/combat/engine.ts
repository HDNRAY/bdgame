...
export interface BattleState {
    phase: BattlePhase; player: Character; opponent: Character; distance: DistanceSystem
    turn: TurnManager; log: BattleLog; eventActorId: string | null; triggerUses: Map<string, number>
}

export class BattleEngine {
    state!: BattleState

    constructor(p: Character, o: Character, d = 4) { this.init(p, o, d) }

    init(p: Character, o: Character, d = 4): void {
        p.resetAp(); o.resetAp()
        const log = new BattleLog(); const tm = new TurnManager()
        tm.addCharacter(p, 0); tm.addCharacter(o, 0)
        log.logBattleStart(p.name, o.name, 0)
        this.state = { phase: 'fighting', player: p, opponent: o, distance: new DistanceSystem(d), turn: tm, log, eventActorId: null, triggerUses: new Map() }
    }
    ...
    private emit(event: TriggerEvent, self: Character, enemy: Character, tMs: number) {
        ...
        const results = processTriggers({ event, actor: self, target: enemy, distance: distance.current }, equipped, triggerUses)
    ...
    startEvent(): boolean {
        ...
        this.emit('turn_start', self, this.state.player.id === e.characterId ? this.state.opponent : this.state.player, e.nextActionAt)
        return true
    }
    ...
    execute(cmd: ActionCommand): ActionResult {
        ...
        switch (cmd.type) {
            case 'move': {
                ...
                if (!self.spendAp(ap)) { log.logSystem(`${self.name} AP不足 无法移动`, tMs); break }
            ...
            case 'attack': {
                ...
                if (!r.hit) { this.emit('on_dodged', self, enemy, tMs); break }

                r.dodged = Math.random() < calcDodgeChance(enemy.attrs.get('dexterity'))
                if (r.dodged) { log.logDodge(self.name, enemy.name, tMs); this.emit('on_dodge', enemy, self, tMs); break }

                const pc = calcParryChance(enemy.attrs.get('strength'), stats.parryRate ?? 0)
                r.parried = Math.random() < pc
                if (r.parried) { log.logParry(self.name, enemy.name, tMs); this.emit('on_parry', enemy, self, tMs) }
                ...
                if (!enemy.isAlive()) { log.logDefeat(enemy.name, self.name, tMs); this.emit('on_kill', self, enemy, tMs); this.state.phase = 'finished' }
                break
            }
